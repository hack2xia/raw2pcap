...
打开页面，粘贴请求（必填）和响应（可选，留空时使用内置的 `200 OK`，响应体为 `generated by raw2pcap`），可选地设置会话中使用的 Client IP / Server IP（Client 默认 `10.10.10.1`；Server IP 留空时取 Host 头中的字面 IPv4，否则 `10.10.10.2`），选择文件名，下载 pcap。
...
`inputRequest` / `inputResponse` 既可以是普通文本字段，也可以是文件（如上）；`clientIp` 可选（默认 `10.10.10.1`），`serverIp` 留空时取 Host 头中的字面 IPv4（域名为 `10.10.10.2`）。
...
可用 `--client-ip` / `--server-ip` 自定义会话中的地址（Client 默认 `10.10.10.1`；不传 `--server-ip` 时取 Host 头中的字面 IPv4，否则 `10.10.10.2`）：
