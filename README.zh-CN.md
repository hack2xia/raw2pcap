...
`serve` 默认绑定 `127.0.0.1`，且 API 没有身份认证，因此请保持 localhost 访问，除非你自行添加保护（见[安全](#安全)）。

打开页面，粘贴请求（必填）和响应（可选，留空时使用内置的 `200 OK`，响应体为 `generated by raw2pcap`），可选地设置会话中使用的 Client IP / Server IP（默认 `10.10.10.1` / `10.10.10.2`），选择文件名，下载 pcap。
...
可用 `--client-ip` / `--server-ip` 自定义会话中的地址（默认 `10.10.10.1` / `10.10.10.2`）：
