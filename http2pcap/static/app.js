// http2pcap web UI logic. All input-shaping logic lives server-side (and is
...
function downloadName(resp) {
  ...
  return m ? m[1] : "http2pcap-result.pcap";
}
...
document.getElementById("f").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const warnings = JSON.parse(resp.headers.get("X-Http2pcap-Warnings") || "[]");
